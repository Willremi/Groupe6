...
class __TwigTemplate_4d864a26458a99b74df4c183e865e592 extends Template
{
...
{% block body %}
    <div class=\"container\">
        {{ include(\x27@WebProfiler/Profiler/header.html.twig\x27, {profile_type: profile_type}, with_context = false) }}
        ...
        {% block summary %}
            {% if profile is defined %}
                ...
                {{ include(\x27@WebProfiler/Profiler/_%s_summary.html.twig\x27|format(profile_type), {
                    profile: profile,
                        ...
                        <div id=\"sidebar-contents\">
                            <div id=\"sidebar-shortcuts\">
                                {% block sidebar_shortcuts_links %}
                                    <div class=\"shortcuts\">
                                        <a class=\"btn btn-link\" href=\"{{ path(\x27_profiler_search\x27, { limit: 10, type: profile_type }) }}\">Last 10</a>
                                        <a class=\"btn btn-link\" href=\"{{ path(\x27_profiler\x27, { token: \x27latest\x27, type: profile_type }|merge(request.query.all)) }}\">Latest</a>
                                        ...
                                        <a class=\"sf-toggle btn btn-link\" data-toggle-selector=\"#sidebar-search\" {% if tokens is defined or about is defined %}data-toggle-initial=\"display\"{% endif %}>
                                            {{ source(\x27@WebProfiler/Icon/search.svg\x27) }} <span class=\"hidden-small\">Search</span>
                                        </a>
                                    </div>
                            ...
                            {% if templates is defined %}
                                <ul id=\"menu-profiler\">
                                    {% if \x27request\x27 is same as(profile_type) %}
                                        {% set excludes = [\x27command\x27] %}
                                    {% elseif \x27command\x27 is same as(profile_type) %}
                                        {% set excludes = [\x27request\x27, \x27router\x27] %}
                                    {% endif %}
                                    ...
                                    {% for name, template in templates|filter((t, n) => n not in excludes) %}
                                        ...
                                        {% if menu is not empty %}
                                            <li class=\"{{ name }} {{ name == panel ? \x27selected\x27 }}\">
                                                <a href=\"{{ path(\x27_profiler\x27, { token: token, panel: name, type: profile_type }) }}\">{{ menu|raw }}</a>
                                            </li>
                                        {% endif %}
                                    {% endfor %}
                                </ul>
