...
class __TwigTemplate_39f63f38cf1c54fd1c2e1c8bb5d136df extends Template
{
...
{% set position = position|default(\x27left\x27) %}
{% set rounded = rounded|default(false) %}
{% set size = size|default(null) %}

{% set classes = [\x27pagination\x27] %}

{% if position != \x27left\x27 %}{% set classes = classes|merge([\x27is-\x27 ~ position]) %}{% endif %}
{% if rounded %}{% set classes = classes|merge([\x27is-rounded\x27]) %}{% endif %}
{% if size != null %}{% set classes = classes|merge([\x27is-\x27 ~ size]) %}{% endif %}

{% if pageCount > 1 %}
    <nav class=\"{{ classes|join(\x27 \x27) }}\" role=\"navigation\" aria-label=\"pagination\">
        {% if previous is defined %}
            <a rel=\"prev\" class=\"pagination-previous\" href=\"{{ path(route, knp_pagination_query(query, previous, options)) }}\">{{ \x27label_previous\x27|trans({}, \x27KnpPaginatorBundle\x27) }}</a>
        {% else %}
            <a class=\"pagination-previous\" disabled>{{ \x27label_previous\x27|trans({}, \x27KnpPaginatorBundle\x27) }}</a>
        {% endif %}

        {% if next is defined %}
            <a rel=\"next\" class=\"pagination-next\" href=\"{{ path(route, knp_pagination_query(query, next, options)) }}\">{{ \x27label_next\x27|trans({}, \x27KnpPaginatorBundle\x27) }}</a>
        {% else %}
            <a class=\"pagination-next\" disabled>{{ \x27label_next\x27|trans({}, \x27KnpPaginatorBundle\x27) }}</a>
        {% endif %}
