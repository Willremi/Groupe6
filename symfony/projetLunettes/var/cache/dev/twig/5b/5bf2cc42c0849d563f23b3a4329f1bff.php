...
class __TwigTemplate_040502ea3a3f4ba7d2631c396c7383d3 extends Template
{
...
";
    ...
    {
        return new Source("<div id=\"sidebar-search\" class=\"{{ (render_hidden_by_default ?? true) ? \x27hidden\x27 }}\">
        ...
        <div class=\"form-group-row\">
            <div class=\"form-group\">
                ...
                <select name=\"method\" id=\"method\">
                    <option value=\"\">Any</option>
                    {% if \x27command\x27 == profile_type %}
                        {% set methods = [\x27BATCH\x27, \x27INTERACTIVE\x27] %}
                    {% else %}
                        {% set methods = [\x27DELETE\x27, \x27GET\x27, \x27HEAD\x27, \x27PATCH\x27, \x27POST\x27, \x27PUT\x27] %}
                    {% endif %}
                    {% for m in methods %}
            ...
            <div class=\"form-group\">
                <label for=\"status_code\">
                    {% if \x27command\x27 == profile_type %}
                        Exit code
                        {% set min_and_max = \x27min=%d max=%d\x27|format(0, 255) %}
                    {% else %}
                        Status
                        {% set min_and_max = \x27min=%d max=%d\x27|format(100, 599) %}
                    {% endif %}
                </label>
