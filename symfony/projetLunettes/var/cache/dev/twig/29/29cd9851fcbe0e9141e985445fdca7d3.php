...
class __TwigTemplate_4798ff3c9890c48ce4ba4ec953153efe extends Template
{
...
{% if request_collector and request_collector.redirect %}
    ...
    {% set redirect_route_name = \x27@\x27 ~ redirect.route %}

    <div class=\"status status-compact status-warning\">
        <span class=\"icon icon-redirect\">{{ source(\x27@WebProfiler/Icon/redirect.svg\x27) }}</span>
...
<div class=\"status {{ css_class }}\">
    {% if status_code > 399 %}
        <p class=\"status-error-details\">
            <span class=\"icon\">{{ source(\x27@WebProfiler/Icon/alert-circle.svg\x27) }}</span>
    ...
    <h2>
        ...
        {% set profile_title = profile.url|length < 160 ? profile.url : profile.url[:160] ~ \x27…\x27 %}
    ...
    <dl class=\"metadata\">
        ...
        {% if referer %}
            <dt></dt>
            <dd>
                <span class=\"icon icon-referer\">{{ source(\x27@WebProfiler/Icon/referrer.svg\x27) }}</span>
        ...
        <dt>Profiled on</dt>
        <dd><time data-convert-to-user-timezone data-render-as-datetime datetime=\"{{ profile.time|date(\x27c\x27) }}\">{{ profile.time|date(\x27r\x27) }}</time></dd>
...
{% if request_collector and request_collector.forwardtoken -%}
    ...
    <div class=\"status status-compact status-compact-forward\">
        <span class=\"icon icon-forward\">{{ source(\x27@WebProfiler/Icon/forward.svg\x27) }}</span>
...
{%- endif %}
", "@WebProfiler/Profiler/_request_summary.html.twig", "D:\\Github\\Groupe6\\symfony\\projetLunettes\\vendor\\symfony\\web-profiler-bundle\\Resources\\views\\Profiler\\_request_summary.html.twig");
    }
}
