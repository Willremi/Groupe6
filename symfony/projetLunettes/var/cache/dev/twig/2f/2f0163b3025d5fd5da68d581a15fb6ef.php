...
class __TwigTemplate_16c8f319a1a328bdf2a91927f67ec736 extends Template
{
...
{% block toolbar %}
    {% if collector.data.unavailable_migrations_count is defined %}
        ...
        {% if unavailable_migrations > 0 or new_migrations > 0 %}
            ...
            {% set status_color = unavailable_migrations > 0 ? \x27yellow\x27 : \x27\x27 %}
            {% set status_color = new_migrations > 0 ? \x27red\x27 : status_color %}

            {% set icon %}
                {% if profiler_markup_version < 3 %}
                    {{ include(\x27@DoctrineMigrations/Collector/icon.svg\x27) }}
                {% else %}
                    {{ include(\x27@DoctrineMigrations/Collector/icon-v3.svg\x27) }}
                {% endif %}
            ...
            {% set text %}
                <div class=\"sf-toolbar-info-group\">
                    <div class=\"sf-toolbar-info-piece\">
                        <b>Current Migration</b>
                        <span>{{ executed_migrations > 0 ? collector.data.executed_migrations|last.version|split(\x27\\\\\x27)|last : \x27n/a\x27 }}</span>
                    </div>
                </div>
                ...
                <div class=\"sf-toolbar-info-group\">
                    ...
                    <div class=\"sf-toolbar-info-piece\">
                        <b>Unavailable</b>
                        <span class=\"sf-toolbar-status {{ unavailable_migrations > 0 ? \x27sf-toolbar-status-yellow\x27 }}\">{{ unavailable_migrations }}</span>
                    </div>
                    <div class=\"sf-toolbar-info-piece\">
                        <b>Available</b>
                    ...
                    <div class=\"sf-toolbar-info-piece\">
                        <b>New</b>
                        <span class=\"sf-toolbar-status {{ new_migrations > 0 ? \x27sf-toolbar-status-red\x27 }}\">{{ new_migrations }}</span>
                    </div>
                </div>
            {% endset %}
...
{% block menu %}
    {% if collector.data.unavailable_migrations_count is defined %}
        {% set unavailable_migrations = collector.data.unavailable_migrations_count %}
        {% set new_migrations = collector.data.new_migrations|length %}
        {% set label = unavailable_migrations > 0 ? \x27label-status-warning\x27 : \x27\x27 %}
        {% set label = new_migrations > 0 ? \x27label-status-error\x27 : label %}
...
{% block panel %}
    ...
    <div class=\"sf-tabs\">
        <div class=\"tab\">
            <h3 class=\"tab-title\">
                Migrations
                <span class=\"badge {{ num_new_migrations > 0 ? \x27status-error\x27 : num_unavailable_migrations > 0 ? \x27status-warning\x27 }}\">
...
{% macro render_migration(migration, profiler_markup_version) %}
    <tr>
        ...
        <td class=\"font-normal align-right\">
            {% if migration.is_new %}
                <span class=\"{{ profiler_markup_version >= 3 ? \x27badge badge-error\x27 : \x27label status-error\x27 }}\">NOT EXECUTED</span>
            {% elseif migration.is_unavailable %}
                <span class=\"{{ profiler_markup_version >= 3 ? \x27badge badge-warning\x27 : \x27label status-warning\x27 }}\">UNAVAILABLE</span>
            {% else %}
                <span class=\"{{ profiler_markup_version >= 3 ? \x27badge badge-success\x27 : \x27label status-success\x27 }}\">EXECUTED</span>
            {% endif %}
        </td>
        <td class=\"font-normal\">{{ migration.executed_at ? migration.executed_at|date(\x27M j, Y H:i\x27) : \x27n/a\x27 }}</td>
        <td class=\"font-normal text-right\">
...
{% endmacro %}
", "@DoctrineMigrations/Collector/migrations.html.twig", "D:\\Github\\Groupe6\\symfony\\projetLunettes\\vendor\\doctrine\\doctrine-migrations-bundle\\templates\\Collector\\migrations.html.twig");
    }
}
