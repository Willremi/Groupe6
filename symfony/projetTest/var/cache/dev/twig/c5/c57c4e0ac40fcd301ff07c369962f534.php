...
class __TwigTemplate_438e0e081d7ec69ad81ccab432d6a999 extends Template
{
    ...
    ";
        ...
        if ((($tmp = Twig\Extension\CoreExtension::length($this->env->getCharset(), CoreExtension::getAttribute($this->env, $this->source, (isset($context["events"]) || array_key_exists("events", $context) ? $context["events"] : (function () { throw new RuntimeError('Variable "events" does not exist.', 6, $this->source); })()), "messages", [], "any", false, false, false, 6))) && $tmp instanceof Markup ? (string) $tmp : $tmp)) {
            ...
            $context["icon"] = ('' === $tmp = \Twig\Extension\CoreExtension::captureOutput((function () use (&$context, $macros, $blocks) {
                // line 8
                yield "            ";
                yield from $this->load("@WebProfiler/Icon/notifier.svg", 8)->unwrap()->yield($context);
                // line 9
...
{% block toolbar %}
    ...
    {% if events.messages|length %}
        {% set icon %}
            {% include(\x27@WebProfiler/Icon/notifier.svg\x27) %}
        ...
        {% set text %}
            ...
            {% for transport in events.transports %}
                <div class=\"sf-toolbar-info-piece\">
                    <b>{{ transport ?: \x27<em>Empty Transport Name</em>\x27 }}</b>
...
{% block menu %}
    ...
    <span class=\"label {{ events.messages|length ? \x27\x27 : \x27disabled\x27 }}\">
        <span class=\"icon\">{{ include(\x27@WebProfiler/Icon/notifier.svg\x27) }}</span>
...
{% block panel %}
    ...
    {% for transport in events.transports %}
        <h3>{{ transport ?: \x27<em>Empty Transport Name</em>\x27 }}</h3>
                                                    ...
                                                    <div class=\"tab-content\">
                                                        <pre class=\"prewrap\" style=\"max-height: 600px\">
                                                            {%- if message.getOptions() is null %}
                                                                {{- \x27(empty)\x27 }}
                                                            {%- else %}
                                                                {{- message.getOptions()|json_encode(constant(\x27JSON_PRETTY_PRINT\x27)) }}
                                                            {%- endif %}
                                                        </pre>
                                                    </div>
                                                </div>
    ...
    {% endfor %}
{% endblock %}
", "@WebProfiler/Collector/notifier.html.twig", "D:\\Github\\Groupe6\\symfony\\projetTest\\vendor\\symfony\\web-profiler-bundle\\Resources\\views\\Collector\\notifier.html.twig");
    }
}
