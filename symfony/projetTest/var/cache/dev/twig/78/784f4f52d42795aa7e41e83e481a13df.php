...
class __TwigTemplate_eda105e18888354bf0f764ed5774fe53 extends Template
{
...
";
    ...
    ";
        // line 22
        if ((($tmp = (isset($context["tokens"]) || array_key_exists("tokens", $context) ? $context["tokens"] : (function () { throw new RuntimeError('Variable "tokens" does not exist.', 22, $this->source); })())) && $tmp instanceof Markup ? (string) $tmp : $tmp)) {
            // line 23
...
{% macro profile_search_filter(request, result, property) %}
    {%- if request.hasSession -%}
        <a href=\"{{ path(\x27_profiler_search_results\x27, request.query.all|merge({token: result.token})|merge({ (property): result[property] })) }}\" title=\"Search\"><span title=\"Search\" class=\"sf-icon sf-search\">{{ include(\x27@WebProfiler/Icon/search.svg\x27) }}</span></a>
    {%- endif -%}
{% endmacro %}
            ...
            <tbody>
                {% for result in tokens %}
                    ...
                    <tr>
                        ...
                        <td>
                            <span class=\"nowrap\">{{ result.ip }} {{ helper.profile_search_filter(request, result, \x27ip\x27) }}</span>
                        </td>
                        <td>
                            <span class=\"nowrap\">{{ result.method }} {{ helper.profile_search_filter(request, result, \x27method\x27) }}</span>
                        </td>
                        <td class=\"break-long-words\">
                            {{ result.url }}
                            {{ helper.profile_search_filter(request, result, \x27url\x27) }}
                        </td>
                        <td class=\"text-small\">
                            <span class=\"nowrap\">{{ result.time|date(\x27d-M-Y\x27) }}</span>
                            <span class=\"nowrap newline\">{{ result.time|date(\x27H:i:s\x27) }}</span>
                        </td>
