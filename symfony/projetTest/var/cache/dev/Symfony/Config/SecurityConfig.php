...
class SecurityConfig implements \Symfony\Component\Config\Builder\ConfigBuilderInterface
{
    ...
    */
     ...
     * @psalm-return (TValue is array ? \Symfony\Config\Security\EncoderConfig : static)
     */
    public function encoder(string $class, string|array $value = []): \Symfony\Config\Security\EncoderConfig|static
    {
        if (!\is_array($value)) {
    ...
    */
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('encoders', $value)) {
            $this->_usedProperties['encoders'] = true;
            $this->encoders = array_map(fn ($v) => \is_array($v) ? new \Symfony\Config\Security\EncoderConfig($v) : $v, $value['encoders']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['encoders'])) {
            $output['encoders'] = array_map(fn ($v) => $v instanceof \Symfony\Config\Security\EncoderConfig ? $v->toArray() : $v, $this->encoders);
        }
