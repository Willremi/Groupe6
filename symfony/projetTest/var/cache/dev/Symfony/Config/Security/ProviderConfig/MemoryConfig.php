...
class MemoryConfig 
{
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('users', $value)) {
            $this->_usedProperties['users'] = true;
            $this->users = array_map(fn ($v) => \is_array($v) ? new \Symfony\Config\Security\ProviderConfig\Memory\UserConfig($v) : $v, $value['users']);
            unset($value['users']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['users'])) {
            $output['users'] = array_map(fn ($v) => $v instanceof \Symfony\Config\Security\ProviderConfig\Memory\UserConfig ? $v->toArray() : $v, $this->users);
        }
