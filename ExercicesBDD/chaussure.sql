...
CREATE TABLE IF NOT EXISTS `chaussure`.`produit` (
  ...
  PRIMARY KEY (`idProduit`, `typeproduit_idTypeProd`, `fournisseur_idFournisseur`, `genreProduit_idGenreProd`),
...
CREATE TABLE IF NOT EXISTS `chaussure`.`utilisateur` (
  ...
  PRIMARY KEY (`idUser`, `adresse_idAdress`, `typeUser_idTypeUser`),
  UNIQUE INDEX `idUser_UNIQUE` (`idUser` ASC),
  INDEX `fk_utilisateur_adresse1_idx` (`adresse_idAdress` ASC),
  INDEX `fk_utilisateur_typeUser1_idx` (`typeUser_idTypeUser` ASC),
  CONSTRAINT `fk_utilisateur_adresse1`
    FOREIGN KEY (`adresse_idAdress`)
    REFERENCES `chaussure`.`adresse` (`idAdress`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_utilisateur_typeUser1`
